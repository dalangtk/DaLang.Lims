...
public static class SqlSugarFilter
{
    ...
    /// 缓存全局查询过滤器（内存缓存）
    /// </summary>
    private static readonly ICacheTool _cache = AppInfo.GetRequiredService<ICacheTool>();
    ...
    public static void DeleteUserOrgCache(long userId, string dbConfigId)
    {
        ...
        // 删除用户机构集合缓存
        _cache.Del($"{CacheKeys.KeyUserOrg}{userId}");
        // 删除最大数据权限缓存
        _cache.Del($"{CacheKeys.KeyRoleMaxDataScope}{userId}");
        // 删除用户机构（数据范围）缓存——过滤器
        _cache.Del($"db:{dbConfigId}:orgList:{userId}");
    }
