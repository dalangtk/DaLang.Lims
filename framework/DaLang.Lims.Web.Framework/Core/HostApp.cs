...
using DaLang.Lims.Web.Framework.Core.Conventions;
using DaLang.Lims.Web.Framework.Core.Dto;
using DaLang.Lims.Web.Framework.Core.Extensions;
...
public class HostApp
{
    ...
    public void Run(string[] args, Assembly assembly = null)
    {
        ...
        try
        {
            ...
            if (appSettings.UseFileConfig)
            {
                AddJsonFilesFromDirectory(configuration, env.EnvironmentName, appSettings.FileConfigPath);
            ...
            {
                //使用系统自带的注入，启动快，但是注入麻烦
                services.RegisterAssemblies(appConfig);
            }
    ...
    private void ConfigureServices(IServiceCollection services, IWebHostEnvironment env, IConfiguration configuration, AppConfig appConfig)
    {
        ...
        .AddScheme<BasicAuthenticationConfig, BasicAuthenticationHandler>("Basic", r =>
        {
            r.UserName = appConfig.BasicAuthentication.UserName;
            r.UserPassword = appConfig.BasicAuthentication.UserPassword;
            r.Realm = appConfig.BasicAuthentication.Realm;
        });
        #endregion 身份认证授权
        ...
        mvcBuilder.Services.Configure<JsonOptions>(options => options.JsonSerializerOptions.AddClayConverters());
